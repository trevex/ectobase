...
// lbIPRec builds one LB_IP PublicPrefix as a backend node announces it: the LB address + its service
// ports (for AddLoadBalancer) alongside this backend's identity (for AddLbBackend).
func lbIPRec(lbIP, owner, overlay string, vni uint32, ports ...LbPort) *rbv1.PublicPrefix {
	return &rbv1.PublicPrefix{
		Kind: rbv1.PublicKind_PUBLIC_KIND_LB_IP, Prefix: lbIP, OwnerUnderlay: owner,
...
func TestApplyPublicLBIP_RegistersLbBeforeBackend(t *testing.T) {
	dp := newRecordingDP()
	b := NewBus("edge1", "fd00:ffff::e1", dp, true)

	b.applyPublic(context.Background(),
		lbIPRec("203.0.113.50/32", "2001:db8::dd", "10.0.0.5", 100, LbPort{Port: 80, Proto: 6}),
		rbv1.RouteOp_ROUTE_OP_ADD)

	reg, ok := dp.lbRegistered["203.0.113.50"]
	if !ok {
		t.Fatalf("LB address was never AddLoadBalancer'd; registered=%+v", dp.lbRegistered)
	}
	...
	if reg.vni != 0 || reg.lbIP != "203.0.113.50" || reg.lbUnderlay != "fd00:ffff::e1" {
		t.Errorf("AddLoadBalancer = %+v, want vni=0 lbIP=203.0.113.50 lbUnderlay=fd00:ffff::e1", reg)
	}
...
// Every backend of an LB address announces the same ports, and the reflector replays the whole snapshot on
...
func TestApplyPublicLBIP_IsIdempotent(t *testing.T) {
	dp := newRecordingDP()
	b := NewBus("edge1", "fd00:ffff::e1", dp, true)
	rec := lbIPRec("203.0.113.50/32", "2001:db8::dd", "10.0.0.5", 100, LbPort{Port: 80, Proto: 6})
	...
	if len(dp.lbIPs) != 1 {
		t.Errorf("AddLoadBalancer called %d times, want 1: %+v", len(dp.lbIPs), dp.lbIPs)
	}
...
// register the LB address again.
func TestApplyPublicLBIP_SecondBackendJoinsExistingLb(t *testing.T) {
	dp := newRecordingDP()
	b := NewBus("edge1", "fd00:ffff::e1", dp, true)
	ctx := context.Background()

	b.applyPublic(ctx, lbIPRec("203.0.113.50/32", "2001:db8::dd", "10.0.0.5", 100, LbPort{Port: 80, Proto: 6}), rbv1.RouteOp_ROUTE_OP_ADD)
	b.applyPublic(ctx, lbIPRec("203.0.113.50/32", "2001:db8::ee", "10.0.0.6", 100, LbPort{Port: 80, Proto: 6}), rbv1.RouteOp_ROUTE_OP_ADD)

	if len(dp.lbIPs) != 1 {
		t.Errorf("AddLoadBalancer called %d times for one LB address, want 1", len(dp.lbIPs))
	}
...
// Withdrawing the LAST backend tears the LB address down too: an edge that Maglev-hashes to an empty
// backend set can only blackhole, and leaving the id registered would make a later re-add fail.
func TestApplyPublicLBIP_LastBackendWithdrawDeletesLb(t *testing.T) {
	dp := newRecordingDP()
	b := NewBus("edge1", "fd00:ffff::e1", dp, true)
	ctx := context.Background()
	a := lbIPRec("203.0.113.50/32", "2001:db8::dd", "10.0.0.5", 100, LbPort{Port: 80, Proto: 6})
	c := lbIPRec("203.0.113.50/32", "2001:db8::ee", "10.0.0.6", 100, LbPort{Port: 80, Proto: 6})
	...
	if _, ok := dp.lbRegistered["203.0.113.50"]; !ok {
		t.Fatal("LB address deleted while a backend remained")
	}

	b.applyPublic(ctx, c, rbv1.RouteOp_ROUTE_OP_WITHDRAW)
	if _, ok := dp.lbRegistered["203.0.113.50"]; ok {
		t.Fatal("LB address still registered after its last backend withdrew")
	}

	// And it must be re-addable afterwards — the proof that the edge's bookkeeping dropped with it.
	b.applyPublic(ctx, a, rbv1.RouteOp_ROUTE_OP_ADD)
	if _, ok := dp.lbRegistered["203.0.113.50"]; !ok {
		t.Fatal("LB address could not be re-registered after a full withdraw")
	}
}
...
func TestApplyPublicLBIP_PortChangeReregistersAndKeepsBackends(t *testing.T) {
	dp := newRecordingDP()
	b := NewBus("edge1", "fd00:ffff::e1", dp, true)
	ctx := context.Background()
	...
	b.applyPublic(ctx, lbIPRec("203.0.113.50/32", "2001:db8::dd", "10.0.0.5", 100,
	...
	if len(dp.lbDels) != 1 {
		t.Errorf("want exactly one DelLoadBalancer for the re-register, got %+v", dp.lbDels)
	}
...
// AddLbBackend, and serve the LB address with an empty backend set until flowplane itself restarted.
func TestApplyPublicLBIP_AdoptsAnLbLeftByAPreviousAgent(t *testing.T) {
	dp := newRecordingDP()
	ctx := context.Background()
	recA := lbIPRec("203.0.113.50/32", "2001:db8::dd", "10.0.0.5", 100, LbPort{Port: 80, Proto: 6})
	recB := lbIPRec("203.0.113.50/32", "2001:db8::ee", "10.0.0.6", 100, LbPort{Port: 80, Proto: 6})

	// A previous agent incarnation registered the LB address and both backends.
	...
	if _, ok := dp.lbRegistered["203.0.113.50"]; !ok {
		t.Fatal("precondition: the previous agent should have registered the LB address")
	}
	...
	if _, ok := dp.lbRegistered["203.0.113.50"]; !ok {
		t.Fatal("LB address is not registered after the agent restart")
	}
	if got := dp.lbBackends["203.0.113.50"]; len(got) != 2 {
	...
	// AddLoadBalancer it holds no record of this LB address, so a backend that scales up next is never attached —
	// and the LB address keeps serving a stale set until flowplane itself restarts.
	recC := lbIPRec("203.0.113.50/32", "2001:db8::ff", "10.0.0.7", 100, LbPort{Port: 80, Proto: 6})
...
// A non-edge node must not touch maglev at all — it reaches an LB address over the plain E/W anycast route.
func TestApplyPublicLBIP_NonEdgeIgnores(t *testing.T) {
	dp := newRecordingDP()
	...
	if len(dp.lbIPs) != 0 || len(dp.lbBackends) != 0 {
		t.Fatalf("non-edge programmed LB state: floating_ips=%+v backends=%+v", dp.lbIPs, dp.lbBackends)
	}
}

// A v6 LB address flows through the same path unchanged (id == LB address, family-agnostic). This is the
// behaviour the retired ReconcileLB's own v6 test guarded.
func TestApplyPublicLBIP_V6Address(t *testing.T) {
	dp := newRecordingDP()
	b := NewBus("edge1", "fd00:ffff::e1", dp, true)

	b.applyPublic(context.Background(),
		lbIPRec("2001:db8:2b::1/128", "2001:db8::dd", "2001:db8:1::5", 100, LbPort{Port: 80, Proto: 6}),
		rbv1.RouteOp_ROUTE_OP_ADD)

	if _, ok := dp.lbRegistered["2001:db8:2b::1"]; !ok {
		t.Fatalf("v6 LB address not registered: %+v", dp.lbRegistered)
	}
}
