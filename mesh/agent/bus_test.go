...
func TestApplyPublic_LBIP_EdgeAddsBackend(t *testing.T) {
	dp := newRecordingDP()
...
func TestApplyPublic_LBIP_NonEdgeIgnores(t *testing.T) {
	dp := newRecordingDP()
	...
	if len(dp.lbBackends) != 0 {
		t.Fatalf("non-edge must ignore LB_IP; got %+v", dp.lbBackends)
	}
}
