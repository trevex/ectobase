...
EDGE_V4=172.29.0.11       # the VyOS edge's clabwan address (returns to nat_ip route here)
...
# routed to the edge, where wan_rx catches them. The host runs no BGP, so this is static.
ip route replace "$NAT_POOL" via "$EDGE_V4" dev "$BR"

echo "clabwan up: $V4 / $V6 ; masquerade $NAT_POOL -> real uplink ; return $NAT_POOL via $EDGE_V4"
