...
fn lb_select_returns_maglev_backend() {
    ...
    let pkt = VecPkt::from_bytes(&tcp_v4([203, 0, 113, 9], lb_ip, 5000, 443));
...
fn lb_select_v6_returns_maglev_backend() {
    ...
    let pkt = VecPkt::from_bytes(&tcp_v6(src6, lb_ip6, 5000, 443));
    assert_eq!(lb_select_forward_v6(&pkt, &m, 0, vni), Some(backend));

    // Non-LB dst (last-4 != LB address key) => None.
