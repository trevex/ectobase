...
//!
//! The conformance suite proves the eBPF bytecode against dpservice; the `flowplane-sim` tests prove
//! the native `flowplane-core` path (`SimNode`). This anchor closes the loop between them: it loads
...
fn uplink_rx_bytecode_matches_native_sim() {
    ...
    //    embeds the bpfel object at `$OUT_DIR/flowplane-prog`; `flowplane` is a binary-only crate with no
