...
- Each cluster also gets a stable anycast API address `fd00:cafe:<h>:1::1`: a health-gated static pod holds it on a `apiaddr0` dummy only while the local apiserver's `/healthz` reports `ok`, and GoBGP advertises `apiaddr0` alongside `dummy0` — `talosctl`/kubectl reach the cluster at this address once it's converged.
...
- **Dispatch cluster** gets the `charts/ectobase-dispatch` Helm chart (aggregated apiserver + controller + kine, the mesh compiler, the shared **reflector**, and the dispatch-side broker identity) — with `-reflector-admin` set to the dispatch's fabric identity via a chart value. Route-bus mTLS is mandatory, so cert-manager is installed on the dispatch cluster first, since the chart's `Issuer`/`ClusterIssuer`/`Certificate` objects need its webhook. The lab then provisions the broker's mTLS enrollment fixtures (dispatch root CA + bootstrap-token kubeconfig) and pre-creates one **ClusterPool** per compute cluster (test fixtures around the install).
- **Each compute cluster** gets the `charts/ectobase-pool` Helm chart (dataplane + agent + broker + cni + pod-materializer; vm-materializer under `lab tier2`) — cert-manager first — wired to the dispatch's reflector at the dispatch's node identity and to its own local apiserver, then the thin **Multus** DaemonSet (installed after the chart, so `flowplane-cni` + the dataplane kubeconfig already exist).
