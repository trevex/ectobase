...
// from the fabric ceph node's `replicapool`; the provisioner runs on the dispatch too
...
func TestRBDPVCBinds(t *testing.T) {
	...
	// The dispatch ceph-csi RBD provisioner must be Running (it drives PVC binds and
	// is also the fence executor).
	require.NoError(t, requireProvisionerRunning(ctx, cfg), "dispatch ceph-csi provisioner not Running")
...
// requireProvisionerRunning asserts dispatch's ceph-csi RBD provisioner pod is
...
func requireProvisionerRunning(ctx context.Context, cfg *config.Config) error {
	phases, err := kubectl(ctx, cfg, "dispatch", "-n", "ceph-csi", "get", "pods",
		"-l", "app=ceph-csi-rbd,component=provisioner", "-o", "jsonpath={.items[*].status.phase}")
	if err != nil {
		return fmt.Errorf("get ceph-csi provisioner pods on the dispatch: %w", err)
	}
	...
	out, err := kubectl(ctx, cfg, "dispatch", "-n", "ceph-csi", "get", "pods",
		"-o", "jsonpath={range .items[*]}{.metadata.name}{\" \"}{.status.phase}{\"\\n\"}{end}")
	if err != nil {
		return fmt.Errorf("list ceph-csi pods on the dispatch: %w", err)
	}
	...
	return fmt.Errorf("no Running ceph-csi-rbd provisioner pod on the dispatch (phases=%q, pods=%q)", phases, out)
}
