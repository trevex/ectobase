...
const (
	// The LB address is IPv6: the fabric's WAN-edge segment (fd00:29::/64) is v6-only, so a WAN client
	// can only reach a v6 LB address. Documentation prefix — not in any overlay/underlay/fabric range.
	lbIP         = "2001:db8:2b::1"
	lbBackendIP4 = "10.0.0.181"
	lbBackendIP6 = "2001:db8:1::181"
	lbBackendMAC = "52:54:00:00:2b:01"

	// lbIP4 is TestLbDistributeSmokeV4's IP: a v4 documentation prefix (RFC 5737
	// TEST-NET-1, fabric.WanPublicV4Test) — the WAN segment is dual-stack as of B10, so a v4
...
// a WAN client curls an IPv6 LB address -> the flowplane wan_rx EDGE (a sidecar in the VyOS edge1 netns)
...
// backend HTTP server replies DSR (src=LB address) -> the backend encaps the reply to the edge's
...
func TestLbDistributeSmoke(t *testing.T) {
	...
	//    dst to the backend's OWN overlay IP (no LB address-on-loopback); the httpd binds that overlay IP.
	...
	// 4. Distributed-LB: register the LB address on the EDGE (vni=0, wan_rx) AND the BACKEND node
	...
	for _, r := range []struct {
		...
		{edge, 0, edgeUnderlay},
		{edge2, 0, edge2Underlay}, // anycast: register the LB address on BOTH edges (WAN ECMPs to either)
		...
		c := r.container
		t.Cleanup(func() { _, _ = dataplaneGRPC(t, ctx, c, "DelLoadBalancer", `{"id":"lb"}`) })
	}

	// 5. WAN client curls the LB address; it is reachable via the edges' public-prefix BGP
	//    advertisement + the WAN's PublicV6→edges ECMP return route (no per-LB-address hack).
	...
	if os.Getenv("LB_HOLD") != "" {
		t.Logf("LB_HOLD set: holding setup up, curling LB address %s in a loop for 15m", lbIP)
		deadline := time.Now().Add(15 * time.Minute)
		for time.Now().Before(deadline) {
			_ = curlFromWan(ctx, wan, lbIP)
	...
	eventually(t, waitDeadline, 5*time.Second, func() error {
		out := curlFromWan(ctx, wan, lbIP)
		if !strings.Contains(out, "hello-lb") {
			return fmt.Errorf("curl of LB address [%s]:80 did not return hello-lb:\n%s", lbIP, out)
		}
		return nil
	})
	t.Logf("N/S IPv6 LB smoke PASS: WAN client curled LB address %s -> backend on %s (edge Maglev + DSR)", lbIP, backend.Cluster)
}
...
// can reach a v4 LB address over the exact same edge Maglev + distributed-LB + DSR path — see
...
func TestLbDistributeSmokeV4(t *testing.T) {
	...
	// 2. Guest netns: UNMODIFIED per the spec's Geneve-TLV DSR (§4.3) — no LB address-on-lo; the edge
	...
	for _, r := range []struct {
		...
		c := r.container
		t.Cleanup(func() { _, _ = dataplaneGRPC(t, ctx, c, "DelLoadBalancer", `{"id":"lb4"}`) })
	}
	// TODO(B11-live): assert Maglev distribution across 2 backends.

	// 5. WAN client curls the v4 LB address; reachable via the edges' public-prefix BGP
	//    advertisement + the WAN's WanPublicV4Test→edges ECMP return route (no per-LB-address hack).
	eventually(t, waitDeadline, 5*time.Second, func() error {
		out := curlFromWanV4(ctx, wan, lbIP4)
		if !strings.Contains(out, "hello-lb") {
			return fmt.Errorf("curl of LB address %s:80 did not return hello-lb:\n%s", lbIP4, out)
		}
		return nil
	})
	t.Logf("N/S IPv4 LB smoke PASS: WAN client curled LB address %s -> backend on %s (edge Maglev + DSR)", lbIP4, backend.Cluster)
}
...
// startLbBackendHTTPD starts a detached IPv6 HTTP server bound to [lbIP]:80 inside the guest netns
...
// kills it. The DSR reply it emits has src=lbIP.
func startLbBackendHTTPD(t *testing.T, ctx context.Context, container, netns, lbIP string) {
	t.Helper()
	startLbBackendHTTPDFamily(t, ctx, container, netns, lbIP, "AF_INET6")
}

// startLbBackendHTTPDv4 is startLbBackendHTTPD's IPv4 sibling: binds lbIP:80 AF_INET
// instead of [lbIP]:80 AF_INET6.
func startLbBackendHTTPDv4(t *testing.T, ctx context.Context, container, netns, lbIP string) {
	t.Helper()
	startLbBackendHTTPDFamily(t, ctx, container, netns, lbIP, "AF_INET")
}
...
func startLbBackendHTTPDFamily(t *testing.T, ctx context.Context, container, netns, lbIP, af string) {
	t.Helper()
...
http.server.HTTPServer((%q,80),H).serve_forever()`, af, lbIP)
...
// curlFromWan curls http://[lbIP]:80/ from inside the wan container's netns via a curl image (the wan
// node ships no HTTP client), returning the combined output.
func curlFromWan(ctx context.Context, wan, lbIP string) string {
	return curlFromWanFamily(ctx, wan, lbIP, true)
}

// curlFromWanV4 is curlFromWan's IPv4 sibling: curls http://lbIP:80/ (no brackets, -4)
// instead of http://[lbIP]:80/ (-6).
func curlFromWanV4(ctx context.Context, wan, lbIP string) string {
	return curlFromWanFamily(ctx, wan, lbIP, false)
}

// curlFromWanFamily is the shared implementation behind curlFromWan and curlFromWanV4.
func curlFromWanFamily(ctx context.Context, wan, lbIP string, v6 bool) string {
	url, flag := fmt.Sprintf("http://%s:80/", lbIP), "-4"
	if v6 {
		url, flag = fmt.Sprintf("http://[%s]:80/", lbIP), "-6"
	}
