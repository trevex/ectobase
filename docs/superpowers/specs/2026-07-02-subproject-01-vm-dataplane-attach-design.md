...
**In scope:** the `cni/` primary-UDN plugin (Go); a clean node-agent gRPC service on the extended Rust `xdp-dp`; minimal IPAM; the monorepo + kind/envtest harness (built to scale 1→N clusters); the e2e acceptance test.
...
- **Node agent = the extended Rust `xdp-dp` daemon** (reuses the working aya datapath: overlay encap/decap, ARP/ND, DHCP, conntrack/NAT).
...
```
Cargo.toml  xdp-dp/ xdp-dp-common/ xdp-dp-ebpf/   # Rust dataplane (extended: new gRPC service)
api/
  proto/
...
```
Served by `xdp-dp` alongside (not replacing) the existing dpservice-compat service. Implementation reuses the existing eBPF datapath modules.

### 5.2 `xdp-dp` extension (Rust)
Add the `DataplaneNode` service; on `AttachInterface`, set up the VM's tap/veth in the target netns and program the eBPF maps/endpoints for that interface (local-only endpoint registration for ①). Reuse `xdp-dp-common` (dhcp, arp_nd) and the egress/overlay logic.

### 5.3 CNI plugin (`cni/`, Go)
A CNI ADD/DEL plugin that, on VMI launch, dials the local `xdp-dp` `DataplaneNode` socket, calls `AttachInterface`, and returns the CNI IPAM result. Wired as the virt-launcher pod's **primary** network (mechanism confirmed by the spike — see §7).
...
- **`xdp-dp` socket exposure** — host unix socket path + how the CNI (running in the kubelet/CNI context) reaches it.
...
3. **Clean node-agent proto** (`api/proto/dataplane/v1`) + `xdp-dp` service stub (returns canned reply) + Rust/Go codegen wired.
4. **CNI plugin** ADD/DEL calling `AttachInterface`; envtest.
5. **Minimal IPAM**.
6. **Real datapath attach** in `xdp-dp` (program eBPF for the interface; DHCP+overlay).
