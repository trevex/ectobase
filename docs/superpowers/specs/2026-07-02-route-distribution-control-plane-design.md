...
- **Control/data separation**: `flowplane` gains only a protocol-agnostic `AddRoute`/`WithdrawRoute`; all CRD/route logic is in Go, off the datapath.
...
| # | Decision | Rationale |
|---|----------|-----------|
| D1 | **Strict control/data split** — `flowplane` dumb + gRPC-driven; Go control plane drives it | Mirrors dpservice←metalnet; keeps CRD/route logic off the Rust datapath; Go is far nicer for controllers/k8s. |
...
2. **Drive local `flowplane`**: `AttachInterface` on create; `AddRoute`/`WithdrawRoute` as bus routes arrive.
3. **Route-bus client**: on a local endpoint, **announce** `(vni, overlay_prefix) → this node's underlay IPv6`; **subscribe by VNI** (only VPCs it hosts); on learning a remote route, program `flowplane`.
...
- `flowplane`: `AddRoute`/`WithdrawRoute` on `dataplane.v1`.
...
- `flowplane/src/…` (Rust) — implement `AddRoute`/`WithdrawRoute`.
