...
- **Makefile:** `docs` → `mkdocs build --strict` (strict = broken nav/links fail the build — the CI gate); `docs-serve` → `mkdocs serve`; `docs-crd-ref` → the per-group `crd-ref-docs` invocations (one per API group, below). Fold `docs-crd-ref` into the `generate` target so the API reference regenerates with the types.
- **Delete:** `docs/book.toml`, `docs/src/SUMMARY.md`, `docs/mermaid.min.js`, `docs/mermaid-init.js`. The DPDK scratch docs (`docs/dpdk-dev.md`, `docs/dataplane/dpdk-b2b-conntrack-nat64-backlog.md`) are **folded and rewritten** into a durable `Architecture/dataplane/dpdk.md` page (see below), then removed — their content is preserved as everlasting prose, not the contemporary/backlog form they're in now.
...
- **Architecture/**
  ...
  - `dataplane/` — `programs.md`, `kernel-xdp-tc.md`, `pure-core.md`, `maps.md` (port `dataplane/*`), and `dpdk.md` ✚ — a durable page on the DPDK dataplane backend (nfkit substrate, the Pkt/Maps backend parity with eBPF/sim, af_xdp/tap, rte_flow offload posture, in-place upgrade). **Folds and polishes** `docs/dpdk-dev.md` + the b2b backlog into everlasting prose (what it is + why + how it relates to the eBPF datapath), status-badged (much is Partial/hardware-gated).
...
- **Reference/**
  - `api/` ✚ — **generated by `crd-ref-docs`, one page per API group**: `net.md`, `compute.md`, `storage.md`, `compiled.md`, `platform.md`. Not hand-edited (regenerated by `make generate`).
  - `crd-interactions.md` ✚ — hand-written: how VPC/NIC/FirewallPolicy/LoadBalancer/NATGateway/VPCPeering/VirtualMachine/Container/Volume/ClusterPool and the `Compiled*` objects relate and flow through the pipeline (the connective tissue the generated per-field ref lacks); links into the per-group generated pages.
...
- `make docs` builds clean under `--strict`; `make docs-serve` works; `make generate` regenerates the per-group `docs/reference/api/{net,compute,storage,compiled,platform}.md` with an otherwise-clean diff.
