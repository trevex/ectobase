...
1. **`Fabric` multi-node sim abstraction** — extend `flowplane-sim` so a test can run a packet across *multiple* nodes (edge → fabric → backend host, and LB reforward origin → LB-home → backend), modelling `bpf_redirect` across the underlay. Returns a `Trace` of every hop for precise assertions.
2. **Full LB datapath port + comprehensive E/W + N/S LB coverage** — port the LB selection/relay path into `flowplane-core` (chiefly one new pure function, `lb_select_forward`, plus `reforward`), then drive both complete flows through the `Fabric` and assert delivery vs. firewall-drop.
...
## 3. `Fabric` multi-node abstraction (`flowplane-sim`)
...
## 4. LB datapath port into `flowplane-core`
...
- **`lb::lb_select_forward<P: Pkt, M: Maps>(pkt, ip_off, vni) -> Option<[u8; 16]>`** — faithful port of the eBPF `lb.rs` primary path: read inner src/dst + L4 ports, `LbKey{vni,dst,port,proto}` lookup, `hash5 % size`, `MaglevKey{table_id,slot}` lookup → backend underlay. (ICMP-error and v6 variants deferred.)
  ...
  - **`parse`:** move `hash5` into `flowplane-core::parse` (already pure).
