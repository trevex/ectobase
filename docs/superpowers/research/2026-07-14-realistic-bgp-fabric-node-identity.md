...
**Phase 1 integration — DONE + validated end-to-end** (`docs/superpowers/plans/2026-07-14-fabric-node-identity-integration.md`, executed): the custom image is wired into the clab fabric (k8s-kind `startup-config` → custom image + per-node `/64` via `extraMounts`; the `host*-frr` sidecars and the dummy0 `clab exec` are gone; `clab-up` builds the image + renders the mount paths). Verified on the live 2-cluster fabric: all node InternalIPs = their fabric `::1` (`fd00:db8:0:{1,2,3}::1`, not docker); in-node FRR sessions to `sw1` **Established** (PfxRcd/PfxSnt=3); cross-cluster underlay routes learned; no sidecars; and the **xdp-dp cross-cluster overlay ping stays 0% loss** — the dataplane works on the fabric-identity nodes.

**Boot-race finding (kindnet vs BGP convergence):** the fabric-identity model gives the CNI a boot-time dependency on peer reachability. kindnet routes each peer's pod-CIDR via that peer's InternalIP — now the fabric `::1` — so at init it needs BGP converged; it panics with "no route to host, Maximum retries" if it loses the race (kindnet's ~5s retry budget < BGP establish + clab wiring `eth1` only after the node boots). It **self-heals**: once BGP is up, a kindnet restart (CrashLoopBackOff) reconciles and the node goes Ready (~≤1 min). We can't gate kubelet on BGP in the clab k8s-kind model (clab wires `eth1` only after kind is Ready → gating would deadlock), so self-heal is the accepted behavior; `kubectl -n kube-system delete pod -l app=kindnet` accelerates it. Worth noting for the dual-homing phase (more uplinks = same race, still self-heals). A patient/BGP-aware CNI or a post-deploy reconcile hook would remove the transient NotReady window.

**Follow-ons (separate plans):** dual-homing (`eth2`+`sw2`, no spine interlink) + active-active egress ECMP (two-uplink `LOCAL` + 50/50 WCMP + per-port ToR MAC from netlink neigh, mirroring dpservice).
