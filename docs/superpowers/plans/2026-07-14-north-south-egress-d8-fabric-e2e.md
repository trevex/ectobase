...
**Refinement (user 2026-07-14) — learn the edge underlay via a dedicated PublicVNI, like LB VIPs.** Today A3 hardcodes `NATGateway.Spec.EdgeUnderlay`. Better (matches spec §3 D5): the edge's agent *announces* its anycast underlay `fd00:db8:0:9::e/128` on a well-known **PublicVNI** via routebus (a small new record type, exactly how LB VIPs would be announced), and the controller/source hypervisors *learn* it — so the egress default route's nexthop is discovered, not configured, and a new edge joining the anycast pool needs no CRD edit. Reuses the routebus reflector fan-out. Sequence this with the LB ingress arc (both want the PublicVNI record). For the D8 e2e, the hardcoded `EdgeUnderlay` is fine; this is the productionization.
