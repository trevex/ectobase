...
**Goal:** Port the LB datapath into `xdp-dp-core`, add a `Fabric` multi-node sim abstraction, make `Compile()` synthesize allow-all for unpolicied NICs (deny-by-default posture), and drive the full E/W + N/S LB test matrix in-process — reproducing and pinning the "LB dropped by the backend firewall" behavior.
...
**Tech Stack:** Rust (`no_std` core, `xdp-dp-sim` std, `etherparse`), Go (controller-runtime), aya eBPF. Guards: `test/conformance/test_lb.py` + `test_flows.py`, a `BPF_PROG_TEST_RUN` anchor, `cargo test -p xdp-dp-sim`.
...
- `LbKey { vni:u32, ipv4:[u8;4], port:u16, proto:u8, _pad:u8 }`, `LbValue { table_id:u32, size:u32 }`, `MaglevKey { table_id:u32, slot:u32 }` — in `xdp-dp-common`. `MAGLEV` map value is **`[u8;16]`** (backend underlay; the `[u8;4]` doc comment is stale).
...
- eBPF `try_uplink_rx` (`ingress.rs`): resolves `u = UNDERLAY[outer_dst]` → `vni`; `lb_ul = lb_select_forward(...)`; branches (NAT64, neighbor-NAT, ICMP-echo) then base FW (245-262, already `xdp_dp_core::firewall::fw_eval_dir`) + conntrack (264-289, skipped for LB) + `decap_and_rewrite` (already `xdp_dp_core::uplink`).
...
- `xdp-dp-core/src/maps.rs` — add `lb_get`/`maglev_get` to `Maps`.
- `xdp-dp-core/src/parse.rs` — add `hash5` (move canonical here; eBPF re-exports).
- `xdp-dp-core/src/lb.rs` (NEW) — `lb_select_forward<P,M>`.
- `xdp-dp-core/src/encap.rs` — add `reforward<P>`.
- `xdp-dp-core/src/uplink.rs` — add the consolidated `uplink_dispatch<P,M>` seam (fw+ct+lb+decap/reforward).
- `xdp-dp-ebpf/src/{coreimpl,parse,lb,encap,ingress}.rs` — `GlobalMaps` lb/maglev; re-export hash5; delegate `lb_select_forward`/`reforward`/`uplink_dispatch`.
- `xdp-dp-sim/src/maps.rs` — `MemMaps` gains `lb`, `maglev`.
- `xdp-dp-sim/src/fabric.rs` (NEW) — `Fabric`, `Hop`, `Trace`, `Outcome`, `NodeId`, `Prog`.
- `xdp-dp-sim/src/sim.rs` — `SimNode` gains `uplink_dispatch`-driven `uplink()` + underlay identity.
- `xdp-dp-sim/src/lb_scenario_test.rs` (NEW) — the §6 matrix.
- `netplane/controllers/compilednic.go` — default-allow generation.
- `xdp-dp/tests/anchor_lb.rs` (NEW) — LB path anchor.

---

## Task 1: `Maps` LB/MAGLEV accessors + sim/eBPF impls

**Files:** Modify `xdp-dp-core/src/maps.rs`, `xdp-dp-sim/src/maps.rs`, `xdp-dp-ebpf/src/coreimpl.rs`. Test: `xdp-dp-sim/src/maps.rs`.

- [ ] **Step 1: Extend the `Maps` trait** — in `xdp-dp-core/src/maps.rs` add to `use` (`LbKey, LbValue, MaglevKey`) and to the trait:
```rust
    fn lb_get(&self, key: &LbKey) -> Option<LbValue>;
    fn maglev_get(&self, key: &MaglevKey) -> Option<[u8; 16]>;
```

- [ ] **Step 2: Write the failing MemMaps test** — append to `xdp-dp-sim/src/maps.rs`:
```rust
#[cfg(test)]
mod tests {
    use super::*;
...
- [ ] **Step 3: Run — verify fail.** `cargo test -p xdp-dp-sim lb_and_maglev_roundtrip` → FAIL (fields/methods missing).

- [ ] **Step 4: Implement in `MemMaps`** — add fields + impl in `xdp-dp-sim/src/maps.rs`:
```rust
...
- [ ] **Step 5: Implement in `GlobalMaps`** — in `xdp-dp-ebpf/src/coreimpl.rs` add to `use` (`LbKey, LbValue, MaglevKey`) and to `impl Maps for GlobalMaps`:
```rust
    #[inline(always)]
...
- [ ] **Step 6: Run.** `cargo test -p xdp-dp-sim lb_and_maglev_roundtrip` → PASS. `cargo build -p xdp-dp` → compiles.

- [ ] **Step 7: Commit.** `git add xdp-dp-core xdp-dp-sim xdp-dp-ebpf && git commit -m "feat(core): Maps lb_get/maglev_get accessors + sim/eBPF impls"`

---

## Task 2: Move `hash5` into core `parse`

**Files:** Modify `xdp-dp-core/src/parse.rs`, `xdp-dp-ebpf/src/parse.rs`.

- [ ] **Step 1: Add `hash5` to `xdp-dp-core/src/parse.rs`** — copy the eBPF `hash5` body verbatim (it is pure; keep the unrolled form and the comment about the verifier):
```rust
...
pub fn hash5(src: &[u8; 4], dst: &[u8; 4], sport: u16, dport: u16, proto: u8) -> u32 {
    // ... EXACT body copied from xdp-dp-ebpf/src/parse.rs::hash5 ...
}
```
(Read `xdp-dp-ebpf/src/parse.rs:95` and copy the full function body.)

- [ ] **Step 2: Re-export from eBPF `parse.rs`** — delete the eBPF `hash5` body and replace with `pub use xdp_dp_core::parse::hash5;`. Verify no other name clash.

- [ ] **Step 3: Run.** `cargo test -p xdp-dp-core -p xdp-dp-sim` → PASS. `cargo build -p xdp-dp` → compiles.

- [ ] **Step 4: Commit.** `git add xdp-dp-core xdp-dp-ebpf && git commit -m "refactor(core): move hash5 into xdp-dp-core::parse; eBPF re-exports"`

---

## Task 3: Port `lb_select_forward` into core

**Files:** Create `xdp-dp-core/src/lb.rs` (+`pub mod lb;`). Modify `xdp-dp-ebpf/src/lb.rs`. Test: `xdp-dp-sim/src/lb_select_test.rs`.

- [ ] **Step 1: Write the core fn** `xdp-dp-core/src/lb.rs`:
```rust
use crate::maps::Maps;
...
```
Add `pub mod lb;` to `xdp-dp-core/src/lib.rs`.

- [ ] **Step 2: Write the failing sim test** `xdp-dp-sim/src/lb_select_test.rs` (`#[cfg(test)] mod lb_select_test;` in sim lib.rs). Uses the `tcp_v4` helper (`pub(crate)` in `firewall_test.rs`):
```rust
...
- [ ] **Step 3: Run — verify pass.** `cargo test -p xdp-dp-sim lb_select` → PASS (size 1 forces slot 0, so the test is deterministic regardless of hash).
...
pub fn lb_select_forward(ctx: &XdpContext, ip_off: usize, vni: u32) -> Option<[u8; 16]> {
    xdp_dp_core::lb::lb_select_forward(&crate::coreimpl::CtxPkt { ctx }, ip_off, vni)
}
```
...
- [ ] **Step 5: Verify.** `cargo test -p xdp-dp-sim lb_select` → PASS. `cargo build -p xdp-dp` → compiles. `nix develop -c bash -c 'CONF_TESTS=test_lb.py ./test/conformance/run.sh'` → PASS (LB datapath unchanged).

- [ ] **Step 6: Commit.** `git add xdp-dp-core xdp-dp-ebpf xdp-dp-sim && git commit -m "refactor(core): port lb_select_forward to core; rewire eBPF"`

---

## Task 4: Port `reforward` into core

**Files:** Modify `xdp-dp-core/src/encap.rs`, `xdp-dp-ebpf/src/encap.rs`. Test: `xdp-dp-sim/src/reforward_test.rs`.

- [ ] **Step 1: Add `reforward` to `xdp-dp-core/src/encap.rs`**:
```rust
...
- [ ] **Step 2: Write the failing sim test** `xdp-dp-sim/src/reforward_test.rs` (`#[cfg(test)] mod reforward_test;`):
```rust
use crate::VecPkt;
use xdp_dp_core::encap::{reforward, ETH_LEN};
...
- [ ] **Step 3: Run — verify pass.** `cargo test -p xdp-dp-sim reforward` → PASS.
...
pub fn reforward(ctx: &XdpContext, local: &Local, lb_underlay: &[u8; 16], backend: &[u8; 16]) -> u32 {
    match xdp_dp_core::encap::reforward(&mut crate::coreimpl::CtxPkt { ctx }, local, lb_underlay, backend) {
        Action::Redirect(ifindex) => unsafe { bpf_redirect(ifindex, 0) } as u32,
        _ => xdp_action::XDP_DROP,
    }
}
```
(Add `use xdp_dp_core::pkt::Action;` as needed. `bpf_redirect` is already imported.)

- [ ] **Step 5: Verify.** `cargo test -p xdp-dp-sim reforward` → PASS. `cargo build -p xdp-dp`. `nix develop -c bash -c 'CONF_TESTS=test_lb.py ./test/conformance/run.sh'` → PASS.

- [ ] **Step 6: Commit.** `git add xdp-dp-core xdp-dp-ebpf xdp-dp-sim && git commit -m "refactor(core): port reforward to core; rewire eBPF"`

---

## Task 5: Consolidated `uplink_dispatch` seam (DELICATE)

**Files:** Modify `xdp-dp-core/src/uplink.rs`, `xdp-dp-ebpf/src/ingress.rs`, `xdp-dp-sim/src/sim.rs`. Guard: full `test_lb.py` + `test_flows.py`.
...
- [ ] **Step 1: Read the source of truth.** Read `xdp-dp-ebpf/src/ingress.rs:100-311` fully — the `lb_ul` computation, the FW block (245-262, gated by `ct miss && DROP && fw_enforcing`), the conntrack block (264-289, `if lb_ul.is_none()`), and the `decap_and_rewrite` delegation (290-310). The seam must reproduce this exactly for the LB-local, LB-remote, and non-LB cases. NAT64/neighbor-NAT/ICMP-echo branches stay in the wrapper BEFORE the seam call.

- [ ] **Step 2: Define the seam** in `xdp-dp-core/src/uplink.rs`:
```rust
...
use xdp_dp_common::{FW_ACTION_DROP, FW_DIR_INGRESS, Local, UnderlayValue};
...
```
Notes: `UnderlayValue` must expose `tap_ifindex` + `guest_mac` (verify field names in `xdp-dp-common`). `decap_and_rewrite` already exists in this module.
...
```rust
match xdp_dp_core::uplink::uplink_dispatch(
...
- [ ] **Step 5: Verify — the critical guard.** `cargo test -p xdp-dp-sim` (all, incl. `ns_scenario_test`) → PASS. `cargo build -p xdp-dp`. `nix develop -c bash -c './test/conformance/run.sh'` (FULL) → must be **93 passed / 2 skipped** (LB + base + everything byte-identical). If conformance regresses, the seam diverged — fix before committing.

- [ ] **Step 6: Commit.** `git add xdp-dp-core xdp-dp-ebpf xdp-dp-sim && git commit -m "feat(core): consolidated uplink_dispatch seam (LB+base); rewire eBPF + sim"`

---

## Task 6: `Fabric` multi-node abstraction

**Files:** Create `xdp-dp-sim/src/fabric.rs` (+`pub mod fabric;`, re-export). Test: in `fabric.rs`.

- [ ] **Step 1: Define the types + `deliver`** in `xdp-dp-sim/src/fabric.rs`:
```rust
...
- [ ] **Step 4: Run.** `cargo test -p xdp-dp-sim fabric` → PASS.

- [ ] **Step 5: Commit.** `git add xdp-dp-sim && git commit -m "feat(sim): Fabric multi-node abstraction (underlay routing + Trace)"`
...
**Files:** Create `xdp-dp-sim/src/lb_scenario_test.rs` (`#[cfg(test)] mod lb_scenario_test;`). Uses `Fabric` + `apply()`.
...
- [ ] **Step 3: Run.** `cargo test -p xdp-dp-sim lb_scenario` → all PASS. **If `ns_lb_delivered_with_allow` (a correct explicit rule) unexpectedly DROPS, that is a genuine datapath bug — stop and investigate via the Trace before "fixing" the test.**

- [ ] **Step 4: Commit.** `git add xdp-dp-sim && git commit -m "test(sim): E/W + N/S LB coverage matrix via Fabric (incl. clab drop reproduction)"`

---

## Task 9: `BPF_PROG_TEST_RUN` anchor for the LB path

**Files:** Create `xdp-dp/tests/anchor_lb.rs`. Modify `Makefile` (extend `sim-anchor` or add `sim-anchor-lb`).

- [ ] **Step 1: Write the anchor** (model on `xdp-dp/tests/anchor_uplink.rs`): load the eBPF object; populate `UNDERLAY`, `LB`, `MAGLEV`, `FW_META`/`FW_RULES`/`FW_CONFIG`, `LOCAL` for an LB-local-deliver fixture (same as sim row 1); craft the encapped-to-backend frame; run `uplink_rx` via the raw `BPF_PROG_TEST_RUN` syscall; build the SAME fixture in a `Fabric`/`SimNode`; assert `out.data == native_pkt` (byte-parity) + action == redirect to the backend tap. `#[ignore]` (privileged).
...
- [ ] **Step 4: Commit.** `git add xdp-dp Makefile && git commit -m "test(anchor): BPF_PROG_TEST_RUN byte-parity anchor for the LB uplink path"`
