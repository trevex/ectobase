...
- **D7 — WAN-edge decap/forward datapath (concrete design from datapath investigation):**
  ...
  - **New eBPF:** the `wan_rx` program + the NEIGHBOR_NAT entry gaining a `vni` field + the vni-agnostic lookup. **Test:** a netns harness (edge with a WAN veth + a fabric veth + a source netns) exercising egress + return both ways BEFORE the fabric e2e. Verifier-sensitive — implement carefully.
