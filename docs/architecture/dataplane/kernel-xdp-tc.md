...
fabric): real NICs behave more forgivingly, which is why a clab-green datapath can still have
latent assumptions.
...
#### Jumbo on clab vs. hardware

On containerlab, compute nodes are pinned to generic/SKB XDP
...
`flowplane serve` probes each uplink's advertised `xdp-features` (`ip -d link
...
`(reason, protocol, ksym(location))` over a failing flow reveals where and why a packet died — the
