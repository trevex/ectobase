...
!!! warning "Status: Partial"
    ...
    KubeVirt network binding plugin backed by the flowplane CNI (`domainAttachmentType=tap`).
...
- The flowplane CNI creates the tap (`domainAttachmentType=tap`, `deviceType=pod-tap`),
  so the device is spliced directly into the flowplane datapath rather than a Linux bridge.
