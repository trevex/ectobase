...
**hub** — the fleet control plane where all intent is authored and compiled —
from N **pools** — the compute clusters that actually run workloads on the shared
eBPF/XDP overlay. The hub decides *what* and *where*; each pool executes its own
...
!!! success "Status: Implemented"
    The single-hub / single-or-many-pool control plane — aggregated apiserver,
...
The hub runs on one ordinary Kubernetes host cluster and adds three moving parts,
all shipped by the `charts/ectobase-hub` chart.
...
aggregated resources (`hub/cmd/apiserver/main.go`):
...
(`charts/ectobase-hub/templates/apiservice.yaml`) — the hub host cluster needs
...
(`hub/pkg/clusterrestriction`), constrains each pool's broker identity so it may
write only its own `ClusterPool` status and may never set `spec.clusterName`.

**Why aggregation, not CRDs, on the hub?** A single aggregated server presents
...
The hub-controller (`hub/cmd/controller/main.go`) runs the fleet reconcilers
against the aggregated apiserver:

- **ClusterPool reconcile** (`hub/pkg/clusterpool`) derives each pool's lifecycle
...
The hub also hosts the **reflector** (`charts/ectobase-hub/templates/reflector.yaml`),
...
Each pool is a plain Kubernetes cluster, registered on the hub as a
`platform.ectobase.dev/ClusterPool`, and provisioned by the
`charts/ectobase-pool` chart. It runs:

- **broker** (`hub/cmd/broker/main.go`) — the kubelet-analog described below.
...
## The hub ↔ pool relationship

The **broker** is the sole seam between a pool and the hub. It is a kubelet-analog:
it watches the hub for the compiled objects stamped with **this pool's**
`spec.clusterName` and reconciles them **down** into the pool's local CRDs.

### Broker sync

The broker (`hub/pkg/broker/broker.go`) runs a **declarative set-reconcile** per
compiled type — `CompiledNIC`, `CompiledVM`, `CompiledVolumeAttachment`,
`CompiledContainer`. On any event it recomputes the desired set (hub objects with
...
To keep watch traffic and memory bounded, the broker's hub cache scopes each
compiled type with a **field selector on `spec.clusterName`**, so its informer
streams only the objects this pool owns (`hub/cmd/broker/main.go`).

Alongside the sync, the broker reports **upward**:

- a **ClusterPool lease + capacity heartbeat** every 10s (the freshness signal the
  hub-controller turns into a phase), summing allocatable resources over Ready
  downstream nodes;
- **fence facts** — each node's `/64` underlay prefix (read from the
  ...
  ClusterPool status and per-VM placement, so the hub can fence precisely on
  failover.

### The broker's hub credential

The broker authenticates to the hub with a dedicated identity, `hub-broker`. The
hub-side chart provisions the ServiceAccount + ClusterRole + Binding
(`charts/ectobase-hub/templates/broker-identity.yaml`), and the pool-side broker
Deployment mounts a kubeconfig **Secret** for it
(`charts/ectobase-pool/templates/broker.yaml`, `--hub-kubeconfig`). The broker
talks to the local pool apiserver in-cluster (its downstream client) and to the
hub through that mounted credential. The `ClusterRestriction` admission plugin
plus the field-scoped cache mean a broker can only ever touch its own pool's slice.

```mermaid
flowchart TB
    subgraph hub["Hub (fleet control plane)"]
        api["aggregated apiserver<br/>(apiserver-kit + kine)<br/>net · compute · storage · compiled · platform"]
        hc["hub-controller<br/>ClusterPool · scheduler · failover/fence"]
...
The hub **compiles and schedules**; pools **execute**. Concentrating intent,
...
partitioned from the hub. This is what bounds hub apiserver load as pools are added.

!!! warning "Status: Partial"
    The **single-cluster execution path** (author intent on the hub → compile →
