...
| `mesh/` | The per-pool Go control plane: `cmd/agent`, `cmd/controller`, `cmd/pod-materializer`, `cmd/reflector`, `cmd/vm-materializer`, plus the `routebus` client/server and the reconcile/desired-state logic. |
...
five control-plane binaries under `cmd/` (agent, controller, pod-materializer, reflector, vm-materializer), the `routebus`
