...
2. Materializes k8s default-allow per direction. k8s semantics are "a NIC no policy
   ...
   would drop everything. `Compile()` therefore emits explicit allow-all rules for both
   families (`0.0.0.0/0` and `::/0`, any proto, any port, `Allow`) for any direction that
   ends up with no rules. A direction a `FirewallPolicy` does govern keeps only that
   policy's rules.
