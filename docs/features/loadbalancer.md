...
delivery. A `LoadBalancer` CRD allocates its VIP from an `LBPool` named by `spec.poolRef`,
lists its service ports, and selects the backend `NetworkInterface`s by selector or by
name. If `spec.vip` is empty the controller allocates the lowest-free address in the pool;
if it is set the controller validates and reserves it. Either way the authoritative value
is `status.allocatedVIP`. LB membership is pure forwarding data; it grants no firewall
permission.
...
```
LoadBalancer { PoolRef, VIP (optional), Ports[], TargetSelector | TargetRefs }
        │  LBVIPReconciler — allocate lowest-free from LBPool, or validate+reserve VIP
        ▼
LoadBalancer.Status.AllocatedVIP
        │  CompiledNICReconciler.Compile()
        │    · match backend NICs (selector or refs)
        │    · consume Status.AllocatedVIP
