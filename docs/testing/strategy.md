...
ectobase asserts each concern at the cheapest level that can observe it.
...
byte-parity anchor: that tests code the production datapath does not run. If
...
output bytes and verdict match. On the Geneve `collect_md` ingress programs, where a
`BPF_PROG_TEST_RUN` skb cannot carry the tunnel key the kernel would stamp on decap,
the anchor instead asserts the bytecode fails safe (`TC_ACT_OK`, packet unchanged)
before it reaches map state. Anchors are deliberately narrow: they exist to catch
