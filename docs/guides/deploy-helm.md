...
!!! success "Status: Implemented"
    ectobase deploys as **two Helm charts** — one for the fleet/hub cluster, one per
...
ectobase is a multi-cluster substrate. A single **hub** cluster runs the control plane (an
aggregated apiserver, the hub controller, the netplane compiler, and the reflector); each
**compute/pool** cluster runs the dataplane, the netplane agent, and a broker that syncs
compiled objects down from the hub. Those two roles map onto the two charts:

| Chart | Runs on | Installs |
|---|---|---|
| `charts/ectobase-hub` | the hub cluster | aggregated apiserver + kine (+ postgres), hub-controller, netplane compiler, reflector, hub-side broker identity |
...
The hub chart carries two namespaces on purpose:

- The **release namespace** (`namespace`, default `system`) holds the baseline-PSA-safe pods:
  the aggregated apiserver, hub-controller, kine, and the hub-side broker identity. Create it
...
```sh
helm install ectobase-hub charts/ectobase-hub \
  --namespace system --create-namespace \
  --set reflectorAdmin='[fd00:cafe:1::1]:1338'
```

`reflectorAdmin` is the address the hub-controller hands to the agents (the `-reflector-admin`
flag); it is the hub's fabric identity where the reflector listens. Point it at the hub's
...
Source of truth: `charts/ectobase-hub/values.yaml` (schema: `values.schema.json`).
...
| `reflectorAdmin` | `[fd00:db8:0:1::1]:1338` | Address passed to the hub-controller as `-reflector-admin` (where the reflector listens). |
| `imagePullPolicy` | `IfNotPresent` | Applied to every container. |
| `images.hubApiserver` | `…/hub-apiserver:dev` | Aggregated apiserver image. |
| `images.hubController` | `…/hub-controller:dev` | Hub controller (ClusterPool reconciler + scheduler). |
...
hub-broker kubeconfig at startup. So two fixtures must exist before `helm install`:
...
2. A **`broker-hub-kubeconfig` Secret** (key `kubeconfig`) holding the broker's credential to
   the hub — a token kubeconfig pointing at the hub's apiserver on the fabric.
...
# 2. broker → hub credential (a token minted for the hub-side hub-broker ServiceAccount)
kubectl create secret generic broker-hub-kubeconfig \
  -n ectobase-system --from-file=kubeconfig=./broker-hub.kubeconfig
...
`broker.clusterName` is the pool's name (must match a `ClusterPool` on the hub) and is
**required**. `apiserverAddress` is *this* cluster's local apiserver (the agent reads/writes
its own cluster); `reflectorAddress` is the hub's reflector on the fabric. The NAD CRD
...
| `reflectorAddress` | `[fd00:db8:0:1::1]:1338` | Hub reflector address the agent dials. |
...
| `broker.hubKubeconfigSecret` | `broker-hub-kubeconfig` | Secret (key `kubeconfig`) with the broker's hub token. |
...
| `images.hubBroker` | `…/hub-broker:dev` | Per-pool broker image. |
...
hub + compute-pool kind fabric — `make lab-up` renders the charts, brings up the clusters,
...
!!! note "Status: Planned"
    The charts are consumed today from the repo tree (`charts/ectobase-hub`,
