...
| `netplane/` | The Go control plane: `cmd/agent`, `cmd/reflector`, `cmd/controller`, plus the `routebus` client/server and the reconcile/desired-state logic. |
| `cni/` | The CNI plugin (`cni/plugin/main.go`) that attaches pods via the `DataplaneNode` gRPC. |
| `api/` | Kubernetes CRD types (`api/v1alpha1/`, group `net.ectobase.dev`) and the gRPC/protobuf contracts (`api/proto/dataplane/v1/`, `api/proto/routebus/v1/`). |
| `config/` | Kubernetes manifests: CRD bases, the `flowplane` DaemonSet, the netplane agent/reflector/controller deployments, RBAC, and the `ectobase-system` namespace. |
...
**native simulator** (`flowplane-sim`). See [The pure-core seam](../dataplane/pure-core.md)
...
[Control/data split & the route bus](../controlplane/route-bus.md) and
[The CRD API](../controlplane/crd-api.md).

## Where to go next

- [Datapath programs](../dataplane/programs.md)
- [The pure-core seam](../dataplane/pure-core.md)
