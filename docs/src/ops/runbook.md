...
(a dead ifindex). Non-edge nodes keep `--pin-links` on (see [HA & restart](./ha-restart.md)).
...
See the [clab + kind fabric](./clab-fabric.md) doc for the fabric-level host/kernel interactions
