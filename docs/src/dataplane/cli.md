...
atomic `bpf_link_update`) is described in [HA & graceful restart](../ops/ha-restart.md).
...
- [The clab + kind fabric](../ops/clab-fabric.md) — where `serve` runs in integration.
- [Getting started](../ops/getting-started.md) — the `make` targets that wrap these.
