...
A [CNI plugin](./architecture/cni-integration.md) wires pods and VM launcher pods into the dataplane at sandbox
creation time.
...
[`CompiledNIC`](./architecture/compile-sync-materialize.md) object — a fully lowered, node-local bundle of that
...
Overlay reachability is distributed by the [route bus](./architecture/route-bus.md): a
...
- Start with **[Architecture → Overview](./concepts/two-planes-and-the-fleet.md)** for the two-plane model and
  the **[overlay](./concepts/overlay.md)** for the wire format.
...
> [Design history](./contributing/index.md).
