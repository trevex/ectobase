...
ectobase is built from two planes and organized as a fleet. Every other concept hangs off
this split.
...
- the fleet, meaning dispatch plus pools — how a single API serves many clusters at once.

## flowplane: the dataplane

flowplane is a Rust eBPF dataplane on tc/tcx. It is deliberately dumb: it holds no policy and makes no
distributed decisions. Every forwarding action is a per-flow-keyed BPF map lookup, and all the
state those maps hold is decided elsewhere and pushed down. It attaches tcx to the kernel `collect_md`
Geneve device and to guest edges, and forwards guest traffic over a
[Geneve overlay](overlay.md) on a routed IPv6 fabric.
...
!!! success "Status: Implemented"
    The eBPF datapath — routing, stateful NAT, Maglev load balancing with DSR, deny-by-default
...
- the reflector — a metalbond-style pub/sub RIB that distributes overlay routes over the route
  bus. Route distribution runs over this pub/sub bus, not BGP; BGP appears only at the
...
`compiled.ectobase.dev`, and `platform.ectobase.dev`. These are aggregated API types,
...
|---|---|---|---|
| Dataplane (flowplane) | Rust (`#![no_std]` eBPF) | every pool node | `tcx` on the geneve device and guest edges |
...
- [The overlay](overlay.md) — the IPv6 underlay and Geneve wire format the dataplane speaks.
