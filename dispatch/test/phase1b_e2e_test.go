...
func TestPhase1b_CompileBindSync_E2E(t *testing.T) {
	...
	// the net types (external api/v1alpha1, registered via netinstall — dispatch's
	...
	if err := dispatchClient.Create(ctx, nicB); err != nil {
	...
	setVNI := func(nic *netv1.NetworkInterface, vni int32) {
		...
		if err := dispatchClient.Get(ctx, client.ObjectKeyFromObject(nic), cur); err != nil {
	...
	if err := dispatchClient.Create(ctx, vm2); err != nil {
		t.Fatalf("dispatch Create vm2: %v", err)
	}
	...
	// ================================================================
	// BIND+SYNC: the c1 broker pulls dispatch -> downstream, bounded by clusterName.
	// ================================================================
