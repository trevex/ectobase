...
//
//   - DISPATCH   = the kit aggregated apiserver (serves CompiledNIC with a
...
func TestBroker_Loopback(t *testing.T) {
	...
	// dispatchEnv is explicitly Stopped in assertion (d) to simulate a dispatch
	...
	// ================================================================
	if err := dispatchClient.Create(ctx, newNIC("nic-a", "c1")); err != nil {
		t.Fatalf("dispatch Create nic-a: %v", err)
	}
	if err := dispatchClient.Create(ctx, newNIC("nic-b", "c2")); err != nil {
		t.Fatalf("dispatch Create nic-b: %v", err)
	}
	...
	if err := dispatchClient.Update(ctx, cur); err != nil {
		t.Fatalf("(b) dispatch Update nic-a: %v", err)
	}

	if err := b.SyncOnce(ctx); err != nil {
		t.Fatalf("(b) SyncOnce: %v", err)
	}
	t.Log("(b) update: PASS (SyncOnce idempotent after dispatch update)")
	...
	// ================================================================
	// (d) PARTITION-SURVIVE: sync nic-c, stop dispatch, downstream persists.
	// ================================================================
	if err := dispatchClient.Create(ctx, newNIC("nic-c", "c1")); err != nil {
		t.Fatalf("(d) dispatch Create nic-c: %v", err)
	}
	if err := b.SyncOnce(ctx); err != nil {
	...
	// Simulate a dispatch outage: stop the dispatch apiserver entirely.
	if err := dispatchEnv.Stop(); err != nil {
		t.Fatalf("(d) dispatch env.Stop: %v", err)
	}
	dispatchStopped = true

	// No SyncOnce here: prove the already-synced state persists without dispatch.
	if keys := downKeys(); len(keys) != 1 || keys[0] != ns+"/nic-c" {
		t.Fatalf("(d) partition-survive: expected downstream=[%s/nic-c] after dispatch outage, got %v", ns, keys)
	}
	t.Logf("(d) partition-survive: PASS (downstream=[%s/nic-c] survives dispatch outage)", ns)

	// Restart dispatch so we can run the CompiledVM assertions.
	dispatchEnv2, err := kitenvtest.NewEnvironment(
		"github.com/trevex/ectobase/dispatch/cmd/apiserver",
		nil,
		[]string{filepath.Join(".", "fixtures")},
	)
	if err != nil {
		t.Fatalf("(e) dispatch NewEnvironment restart: %v", err)
	}
	if _, err := dispatchEnv2.Start(scheme, os.Stderr); err != nil {
		t.Fatalf("(e) dispatch env2.Start: %v", err)
	}
	t.Cleanup(func() {
		if err := dispatchEnv2.Stop(); err != nil {
			t.Errorf("dispatch env2.Stop: %v", err)
		}
	})
	if err := dispatchEnv2.WaitUntilReadyWithTimeout(apiServiceTimeout); err != nil {
		t.Fatalf("(e) dispatch env2 WaitUntilReadyWithTimeout: %v", err)
	}
	dispatchClient2, err := client.New(dispatchEnv2.GetRESTConfig(), client.Options{Scheme: scheme})
	if err != nil {
	...
	// ================================================================
	if err := dispatchClient2.Create(ctx, newVM("vm-a", "c1", "fedora")); err != nil {
		t.Fatalf("(e) dispatch Create vm-a: %v", err)
	}
	if err := dispatchClient2.Create(ctx, newVM("vm-b", "c2", "ubuntu")); err != nil {
		t.Fatalf("(e) dispatch Create vm-b (c2): %v", err)
	}
	...
	if err := dispatchClient2.Update(ctx, curVM); err != nil {
		t.Fatalf("(e) dispatch Update vm-a: %v", err)
	}
	if err := b2.SyncCompiledVMs(ctx); err != nil {
	...
	// ================================================================
	if err := dispatchClient2.Create(ctx, newAtt("att-a", "c1", "fedora")); err != nil {
		t.Fatalf("(f) dispatch Create att-a: %v", err)
	}
	if err := dispatchClient2.Create(ctx, newAtt("att-b", "c2", "ubuntu")); err != nil {
		t.Fatalf("(f) dispatch Create att-b (c2): %v", err)
	}
	...
	if err := dispatchClient2.Update(ctx, curAtt); err != nil {
		t.Fatalf("(f) dispatch Update att-a: %v", err)
	}
	if err := b2.SyncCompiledVolumeAttachments(ctx); err != nil {
	...
	// ================================================================
	if err := dispatchClient2.Create(ctx, newCtr("ctr-a", "c1", "nginx")); err != nil {
		t.Fatalf("(g) dispatch Create ctr-a: %v", err)
	}
	if err := dispatchClient2.Create(ctx, newCtr("ctr-b", "c2", "redis")); err != nil {
		t.Fatalf("(g) dispatch Create ctr-b (c2): %v", err)
	}
	...
	if err := dispatchClient2.Update(ctx, curCtr); err != nil {
		t.Fatalf("(g) dispatch Update ctr-a: %v", err)
	}
	if err := b2.SyncCompiledContainers(ctx); err != nil {
