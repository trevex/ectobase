...
func TestCeph_ScheduleCompileSyncMaterializeVolume_E2E(t *testing.T) {
	...
	if err := dispatchClient.Get(ctx, client.ObjectKey{Name: "c1"}, gotPool); err != nil {
	...
	if err := dispatchClient.Create(ctx, boot); err != nil {
		t.Fatalf("dispatch create volume boot: %v", err)
	}
