...
func TestHeartbeatOnce(t *testing.T) {
	...
	h := &Heartbeater{Dispatch: c, PoolName: "c1", HolderIdentity: "broker-1", Reporter: staticReporter{rl}}
...
func TestHeartbeatOnce_ReporterError(t *testing.T) {
	...
	h := &Heartbeater{Dispatch: c, PoolName: "c1", HolderIdentity: "broker-1", Reporter: errReporter{errors.New("boom")}}
	if err := h.heartbeatOnce(context.Background()); err == nil {
...
func TestHeartbeatOnce_PoolNotFound(t *testing.T) {
	...
	h := &Heartbeater{Dispatch: c, PoolName: "missing", HolderIdentity: "b", Reporter: staticReporter{}}
	if err := h.heartbeatOnce(context.Background()); err == nil {
