...
docker rm -f dispatch-pg dispatch-kine >/dev/null 2>&1 || true

docker run -d --name dispatch-pg -e POSTGRES_PASSWORD=kine -e POSTGRES_DB=kine -p 5432:5432 "$PG_IMAGE" >/dev/null
# wait for postgres
for i in $(seq 1 30); do docker exec dispatch-pg pg_isready -U postgres >/dev/null 2>&1 && break; sleep 1; done

docker run -d --name dispatch-kine --network host "$KINE_IMAGE" \
